...
- **Afordancia de tab activa**: cada ícono usa la variante outline cuando el tab no está activo y la variante filled cuando sí (`film`/`film-outline`, `tv`/`tv-outline`, `star`/`star-outline`) — un patrón común para comunicar "en qué sección estoy" sin depender solo del color.
- **Jest**: `@expo/vector-icons` importa `expo-font`, que arrastra `expo-modules-core` y con eso el mismo problema que ya había aparecido con `expo-image` (`EventEmitter` nativo no disponible en el entorno Node de Jest, ver sección "Testing E2E"/CI). Mismo tipo de solución: un mock manual (`__mocks__/vector-icons.js`) que resuelve cualquier set de íconos a un `View` que reenvía sus props (`name`, `size`, `color`) sin tocar código nativo — así los tests pueden seguir aserting sobre el prop `name` (p. ej. `star` vs. `star-outline`) sin necesitar el módulo real.
