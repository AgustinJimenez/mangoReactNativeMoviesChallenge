...
src/
  api/
    index.ts           # RTK Query apiSlice: getPopularMovies, searchMovies, getMovieDetails,
...
Quedó afuera de este documento en la primera pasada — se decidió sobre la marcha durante el desarrollo, no en la etapa de planning original.

- **Punto de partida**: los íconos de la tab bar (🎬/📺/⭐) y el de favorito (★/☆) arrancaron como glyphs de emoji Unicode planos en un `<Text>`, sin ninguna dependencia nueva — la misma lógica que llevó a NativeWind sobre otras opciones de estilos: minimizar superficie de módulos nativos dado lo mucho que costó ya el problema de `react-native-fast-image`.
- **Cambio a `@expo/vector-icons`**: una vez que el resultado visual de los emoji no alcanzaba (inconsistentes entre sí, no reflejan estado "activo" de forma clara), se migró a `@expo/vector-icons` (set `Ionicons`) para la tab bar, `FavoriteButton` y el ícono de `RatingBadge`. El riesgo de sumar un módulo nativo nuevo acá es bajo porque `expo-modules-core` **ya era una dependencia** (por `expo-image`) — sumar `expo-font` + `expo-asset` (peer deps de `@expo/vector-icons`) es agregar dos módulos más al mismo mecanismo de autolinking ya probado, no adoptar infraestructura nueva.
- **Afordancia de tab activa**: cada ícono usa la variante outline cuando el tab no está activo y la variante filled cuando sí (`film`/`film-outline`, `tv`/`tv-outline`, `heart`/`heart-outline`) — un patrón común para comunicar "en qué sección estoy" sin depender solo del color.
- **Jest**: `@expo/vector-icons` importa `expo-font`, que arrastra `expo-modules-core` y con eso el mismo problema que ya había aparecido con `expo-image` (`EventEmitter` nativo no disponible en el entorno Node de Jest, ver sección "Testing E2E"/CI). Mismo tipo de solución: un mock manual (`__mocks__/vector-icons.js`) que resuelve cualquier set de íconos a un `View` que reenvía sus props (`name`, `size`, `color`) sin tocar código nativo — así los tests pueden seguir aserting sobre el prop `name` (p. ej. `heart` vs. `heart-outline`) sin necesitar el módulo real.
- **Inconsistencia corregida**: `FavoriteButton` (el toggle de favorito en cada fila de lista y en el header de detalles) arrancó con `star`/`star-outline`, distinto del `heart`/`heart-outline` de la tab de Favoritos — y peor, ese mismo `star` colisionaba visualmente con el ícono de rating de `RatingBadge` (también `star`, pero con un significado totalmente distinto: puntaje de TMDB, no favorito), quedando dos estrellas de significado distinto una al lado de la otra en cada fila de lista. Se unificó `FavoriteButton` a `heart`/`heart-outline` para que coincida con la tab y deje de superponerse visualmente con el ícono de rating.
...
| `@expo/vector-icons` (+ `expo-font`, `expo-asset`) | Expo | Bajo — mismo ecosistema/mecanismo de autolinking que `expo-image`, ya probado en el proyecto (ver "Iconografía"). |
...
- `api/index.ts`: tipos de request/response crudos de TMDB (antes de normalizar a `Media`), privados del módulo salvo que otro módulo los necesite.

Excepción: `types/common.ts`, un archivo chico para los pocos tipos genuinamente **cross-cutting** que no son dueños de ningún módulo — `MediaType = 'movie' | 'tv'` (lo usan movies, tv y favorites por igual), `Locale = 'es' | 'en'` (lo usan `i18n/` y `api/index.ts`), y un genérico `PaginatedResponse<T>` si varios endpoints de TMDB comparten esa forma.
...
- **Qué se persiste**: el reducer de `api` (RTK Query) vía `redux-persist`, con un transform que guarda solo el sub-estado `queries` (los resultados ya resueltos) — no `subscriptions` ni `config`, que son estado transitorio de la librería y no aportan nada persistidos.
- **Comportamiento al reabrir la app**: si ya existe una entrada de caché para la query actual (mismo endpoint + params + idioma, que es el cache key), la pantalla se pinta de inmediato con esos datos — sin `LoadingState`/skeleton de pantalla completa. Esto aplica tanto a las listas (`ListTemplate`) como a la página de detalles (`DetailsTemplate`) — ambas leen `isLoading`/`isFetching`/`isError` de la query y arman su UI igual.
  - **Nota de proceso**: esto quedó como texto aspiracional sin implementar durante una vuelta de la sesión — `api/index.ts` no tenía `refetchOnMountOrArgChange` ni `setupListeners`, así que aunque el dato ya persistido se mostraba al instante (eso sí funcionaba, vía `redux-persist`), no había ningún refetch real en segundo plano en ningún lado, ni en listas ni en detalles. Se detectó al releer este documento contra el código real y se corrigió (ver el punto siguiente) — queda como recordatorio de por qué conviene verificar contra el código antes de confiar en lo que dice este archivo.
- **`refetchOnMountOrArgChange: 300`** (top-level, en `createApi()`, `api/index.ts`) — si una query se remonta con los mismos argumentos y la entrada cacheada tiene más de 5 minutos, dispara un refetch real en segundo plano (sin bloquear la UI, ya que `data` sigue disponible mientras tanto). Aplica a las cuatro queries (listas, búsqueda, detalles) por igual, a nivel de API completa en vez de repetir la opción en cada endpoint. 300s es lo bastante generoso para que navegar de ida y vuelta dentro de la misma sesión (lista → detalle → atrás → mismo detalle) nunca dispare un refetch innecesario, pero sigue cubriendo el caso real de "cerré la app y volví más tarde".
- **Indicador de refresh en segundo plano**: se usa `isFetching` (true en cualquier fetch en curso, tenga o no datos previos) para mostrar `RefreshIndicator` (`ui/atoms/`) — una barra de progreso fina, no bloqueante — en vez de re-mostrar el spinner/skeleton grande. En `DetailsTemplate` va **después** de `MediaDetailsHeader`, no antes — el `Backdrop` necesita quedar pegado al borde superior real del `ScrollView` para que el header transparente del stack siga flotando sobre la imagen (ver sección de header transparente); ponerlo antes reintroducía el espacio muerto que esa sección resolvió.
- **Si el refetch en segundo plano falla**: como ya hay `data` (la cacheada), no se muestra `ErrorState` — se deja el contenido viejo visible con un aviso chico (`common.staleDataNotice`, compartido entre `ListTemplate` y `DetailsTemplate`) en vez de bloquear la pantalla.
- **`keepUnusedDataFor`**: sin override, usa el default de RTK Query (60s) — no afecta lo persistido en sí (eso depende de que `redux-persist` haya escrito a `AsyncStorage` antes de que la app se cierre, algo que hace con debounce en cada cambio de store), solo cuánto tiempo se mantiene la caché *en memoria* mientras la app está en primer plano sin esa pantalla montada.
...
## Filtro por género y orden

Agregado en una vuelta posterior de la implementación (adaptando un mockup de diseño), no estaba en el plan original.

- **`/movie/popular` y `/tv/popular` no soportan `with_genres` ni `sort_by`** — según la propia doc de TMDB, esos endpoints son "básicamente un discover call detrás de escena" pero con parámetros fijos (solo `page`/`language`/`region`). Para que el filtro de género y el orden funcionen de verdad (no solo decorativos), `getPopularMovies`/`getPopularTv` (`api/index.ts`) pasaron de `/movie/popular` y `/tv/popular` a `/discover/movie` y `/discover/tv`, que sí soportan ambos parámetros. Con `sort_by: 'popularity.desc'` por default (el mismo default de `/movie/popular`), el cambio es un no-op hasta que el usuario elige un filtro u orden.
- **`SortBy`** (`'popularity' | 'rating' | 'newest' | 'title'`) es un tipo compartido movie/tv, pero el parámetro TMDB real difiere por tipo: movies ordenan por `primary_release_date`, tv por `first_air_date` (fecha de estreno vs. fecha de primer episodio). `toMovieSortByParam`/`toTvSortByParam` en `api/index.ts` hacen ese mapeo.
- **`serializeQueryArgs` incluye `genreId`/`sortBy`** en la cache key de RTK Query, igual que ya incluía `query` en las búsquedas — sin esto, cambiar de filtro reusaría (incorrectamente) la caché paginada de otro filtro.
- **El buscador (`/search/movie`, `/search/tv`) no soporta estos parámetros** — TMDB no tiene un "discover-search" combinado. Por eso `GenreFilterRow` y `SortSelect` (ambos en `ui/molecules/`) solo se muestran cuando no hay una búsqueda activa (`ListTemplate` calcula `isSearching` a partir de `searchValue` y oculta el bloque de filtros mientras tanto) — mostrarlos sin efecto durante una búsqueda sería UI engañosa.
- **`MOVIE_GENRE_IDS`/`TV_GENRE_IDS`** (`utils/genres.ts`) son el orden de iteración de los chips, derivado de los mismos mapas ID→clave i18n que ya existían para resolver nombres de género en `MediaListItem` — sin una lista nueva que duplique los IDs a mano. Ojo: como las claves de esos mapas son numéricas, el spec de JS las itera en orden ascendente sin importar el orden de inserción en el objeto — no hay forma de curar un orden "amigable" (Action, Adventure, Comedy…) sin una lista aparte; se aceptó el orden ascendente por simplicidad.
- **Reset de página**: igual que el buscador, cambiar de género o de orden resetea `page` a 1 (si no, la próxima página pedida seguiría la paginación del filtro anterior).
- **Descomposición de `ListTemplate`**: agregar el bloque de filtros hizo que el archivo/función superaran el límite de líneas del lint (`max-lines`/`max-lines-per-function`) — se extrajeron `ListHeader.tsx` (título/subtítulo/selector de idioma), `ListBody.tsx` en `ui/organisms/` (los estados loading/error/empty + el `FlatList` animado) y `ListFilters.tsx` (chips de género + dropdown de orden) como componentes propios, dejando `ListTemplate` como pura composición de los cuatro bloques (header, buscador, filtros, body).
...
## Página de detalles: enriquecimiento (backdrop, cast, tráiler, recomendaciones)

Agregado en una vuelta posterior de la implementación (a partir de un mockup de referencia), no estaba en el plan original — la versión inicial de `MediaDetailsHeader`/`DetailsTemplate` solo tenía poster + título + fecha + géneros + sinopsis.

- **`append_to_response=credits,videos,recommendations`** en `getMovieDetails`/`getTvDetails` (`api/index.ts`) — TMDB permite pedir sub-recursos relacionados en la misma llamada de detalles en vez de tres requests separados (`/movie/{id}/credits`, `/videos`, `/recommendations`). Mismo costo de red que antes, mucha más data.
- **`movieDetailsToMediaDetails`/`tvDetailsToMediaDetails`** reemplazan la vieja `toMediaDetails` genérica compartida — una vez que movies (`runtime: number`) y tv (`episode_run_time: number[]`) divergen en cómo representan la duración, ya no tenía sentido una función única con 5 parámetros posicionales; cada mapper arma su propio `MediaDetails` completo, con helpers compartidos (`toCastMembers`, `findTrailerKey`) para lo que sí es idéntico entre ambos.
- **Cast**: `CastMember[]` capado a `MAX_CAST_MEMBERS = 10` (constante en `api/index.ts`) — es una tira de "elenco principal" en la página de detalles, no una lista de créditos completa; no hay pantalla de "ver todos" (se decidió no construir esa UI sin la navegación real detrás).
- **Tráiler**: `findTrailerKey` prioriza YouTube + tipo "Trailer" + oficial, con fallbacks (cualquier trailer de YouTube, después cualquier video de YouTube) — bastantes títulos no tienen un video marcado exactamente como "Trailer" oficial. Sitios que no son YouTube (Vimeo) se descartan porque `TrailerSection` solo sabe armar un link de youtube.com. Si no hay ningún video de YouTube, `trailerKey` es `null` y `TrailerSection` no se renderiza — decisión explícita de UX (ver `AskUserQuestion` en la sesión): abrir en la app/sitio de YouTube externo vía `Linking.openURL`, no un reproductor embebido (evita sumar `react-native-webview` como dependencia nativa nueva).
- **Recomendaciones**: `recommendations: Media[]` normalizado con los mismos `movieToMedia`/`tvToMedia` que usan las listas — tocar una card navega (`navigation.push`, no `navigate`) al details screen del mismo tipo de media, agregando una screen nueva al stack en cada tap para poder volver atrás por la cadena completa de recomendaciones. Como el endpoint de recomendaciones de un movie solo devuelve movies (y tv solo tv), nunca hay problema de cross-stack: el stack actual siempre tiene registrada su propia ruta de detalles.
- **Compartir**: `ShareButton` usa la API nativa `Share.share()` de React Native (no hay backend ni librería nueva) — se muestra en el header nativo del stack (`headerRight`), no en el body scrolleable. `useDetailsHeaderShare` recibe el `MediaDetails` completo y setea `headerRight` una vez que la query resuelve (`useLayoutEffect`).
- **Runtime**: `formatRuntime` (`utils/runtime.ts`) da formato "1h 52m" / "2h" / "45m" sin i18n — "h"/"m" se entienden igual en ambos locales de la app, y esto evita pluralización con ICU que el proyecto ya había descartado para otros casos (ver sección de i18n).
- **Header transparente sobre el backdrop**: `stackScreenOptions.details` (`navigation/screenOptions.tsx`) usa `headerTransparent: true` + `headerStyle: { backgroundColor: 'transparent' }` (el segundo es necesario aparte del primero — el `headerStyle.backgroundColor` opaco que setea el `screenOptions` compartido del stack no se anula solo con `headerTransparent`) para que el back arrow y el botón de compartir floten sobre la imagen de `Backdrop` en vez de apilarse en una barra sólida separada (así lo mostraba el mockup de referencia). Se probó primero con un scrim plano de ancho completo sobre `Backdrop` para darle contraste a los íconos, pero tapaba demasiado de la imagen — se reemplazó por un fondo circular semi-transparente (`bg-black/25`) en cada ícono (`BackButton.tsx`, `ShareButton.tsx`) en vez de una franja entera. `headerLeft` es un `BackButton` custom (no el back arrow default de native-stack, que no trae fondo propio). El header **no** setea `title` — al ser transparente y quedar fijo mientras el `ScrollView` se desplaza debajo, un título ahí colisionaba visualmente con el título real (`MediaDetailsHeader`/`MediaDetailsInfo`) una vez que el backdrop salía de vista por el scroll; el título ya se ve prominente debajo del backdrop, así que el header nativo solo lleva los íconos.
- **Poster superpuesto sobre el backdrop**: `MediaDetailsHeader` (a partir de otro mockup de referencia, un componente React Native standalone) aplica `marginTop: -POSTER_OVERLAP` (64dp) al bloque poster+título+géneros en vez de dejarlo apilado con un gap plano debajo del backdrop — el poster (128dp de ancho, `aspect-[2/3]` → 192dp de alto) queda parcialmente montado sobre la imagen, con un borde `border-white/30` (del mismo mockup) para separarlo visualmente del backdrop. No hace falta gradiente/scrim extra ahí: la columna de título/rating usa `justify-end` dentro de una fila estirada a la altura del poster, así que su contenido (más bajo que 192dp) siempre cae por debajo del borde inferior del backdrop, sobre el fondo sólido — solo el poster (opaco) pisa la imagen.
- **Clasificación por edad (certification)**: TMDB no la trae en el endpoint de detalles ni es un valor único — es per-país, vía `append_to_response=release_dates` (movies) o `content_ratings` (tv), cada uno con una entrada por país (`release_dates` además anida varias fechas de estreno por país, solo algunas con `certification` no vacío — `findMovieCertification` toma la primera no vacía; `content_ratings` es plano, un rating por país). `toTmdbRegion` (`i18n/tmdbLocale.ts`) deriva el país (`US`/`ES`) de la misma tabla `es→es-ES`/`en→en-US` que ya usa `toTmdbLanguage`, en vez de mantener una tabla de regiones aparte — así la certificación siempre corresponde al mismo país cuyo idioma/fechas ya se están pidiendo. Se muestra como un badge con borde junto a la fecha en `MediaDetailsHeader`; `null` cuando TMDB no tiene certificación para esa región (bastante común fuera de US), y el badge simplemente no se renderiza.
...
- **Sincronización con TMDB**: el idioma activo de `i18next` se mapea a un código TMDB (`es` → `es-ES`, `en` → `en-US`) y se inyecta como parámetro `language` en todos los endpoints de `api/index.ts`. Al cambiar de idioma se invalida la caché de RTK Query correspondiente (las queries usan el idioma como parte del cache key, así que esto es automático).
...
- **Unit**: `favoritesSlice` (reducers/selectors), helpers de `utils/image.ts`, normalización de `Media` en `api/index.ts`, mapeo de idioma `i18next` → parámetro TMDB.
...
- `@typescript-eslint/naming-convention`: camelCase para variables/funciones, PascalCase para tipos/interfaces/enums/componentes, `UPPER_CASE` permitido para constantes de módulo. **Sin regla** sobre `objectLiteralProperty`/`typeProperty` — TMDB devuelve `release_date`, `first_air_date`, `vote_average` en snake_case, y forzar camelCase ahí haría pelear al linter contra la forma cruda de la API antes de normalizarla a `Media` en `api/index.ts`.
...
Esta sección se escribió antes de implementar, como registro de qué estaba incierto en el momento de planificar. Se deja el texto original de cada punto (tachado si se resolvió) con una nota de estado al final — varios quedaron sin asignar a ningún paso numerado del plan de trabajo pese a estar acá, y por eso no se cerraron hasta una pasada de verificación tardía (o no se cerraron nunca). Ver `CLAUDE.md` para el gotcha general que esto ilustra: un pendiente anotado acá no se resuelve solo por estar escrito, necesita quedar asignado a un paso concreto.

- ~~Confirmar `react-native-dotenv` vs `react-native-config` una vez se inicialice el proyecto~~ — **resuelto**: `react-native-dotenv` (ver `babel.config.js`, paso 1).
- ~~Decidir si "top_rated" o "popular" es el endpoint por defecto~~ — **resuelto**: `popular` (ver `api/index.ts`).
- ~~Definir criterio exacto de normalización `Media`~~ — **resuelto**: implementado y testeado (`api/index.ts` + `api/index.test.ts`, paso 15).
- Confirmar si Hermes necesita el polyfill de `Intl.PluralRules` para `i18next` — **no se llegó a ejercitar**: ninguna clave de traducción usa formas plurales (`_plural`/`count`), así que este código path de i18next nunca corrió. Si se agrega un texto con plural en el futuro, confirmar ahí.
- ~~Decidir dónde vive el `LanguageSwitcher`~~ — **resuelto, pero tarde**: `headerRight` de cada stack de listado (paso 16, no en el paso 8 donde se construyó el componente). Se había anotado acá como pendiente pero ningún paso numerado lo tenía asignado explícitamente, así que quedó un componente construido y sin usar hasta que una verificación interactiva lo notó — no un problema de código, un hueco en el plan mismo.
- ~~Instalar Android Studio/SDK y crear un AVD~~ — **resuelto**: SDK en `~/Library/Android/sdk`, AVD `Pixel_8_API36` corriendo durante todo el desarrollo. `mobile-mcp` se registró (paso 16) pero no llegó a cargarse en la sesión (requiere reiniciar la sesión de Claude Code para tomar un MCP server nuevo) — la verificación interactiva se hizo igual, vía `adb` directo.
- ~~**TODO**: unificar `MoviesListScreen`/`TvListScreen` (y sus detalles) en una page parametrizada por `mediaType`~~ — **resuelto**: `useMediaListState`/`useMediaListScreen`/`useMediaDetailsScreen` (`src/hooks/`) ahora concentran toda la lógica compartida — estado de búsqueda/paginación/filtros, armado de las props de `ListTemplate`/`DetailsTemplate`, el efecto del botón de compartir. Cada screen quedó en ~15-20 líneas: solo llama a sus propios hooks de RTK Query (`useGetPopularMoviesQuery`/`useGetPopularTvQuery`, etc. — no se pueden unificar, son hooks generados distintos por endpoint) y delega el resto. Los cuatro tests de pantalla originales (sin modificar) siguen pasando, confirmando que el refactor no cambió comportamiento.
- ~~Verificar que NativeWind compila sin fricción en Android e iOS~~ — **parcial, y ya no es un pendiente real**: confirmado extensivamente en Android; iOS nunca se compiló en esta máquina (sin Xcode instalado). El entrevistador confirmó que la entrega solo necesita Android — no es una limitación que haga falta seguir mencionando como abierta (sí se mantiene documentada tal cual en el README, sin la aclaración del entrevistador, ya que eso es contexto de la conversación de evaluación, no del código).
- ~~**TODO (verificación manual pendiente)**: probar a mano el escenario completo de caché offline~~ — **resuelto**: verificado en vivo contra el emulador real (`svc wifi disable` + `svc data disable` con la app ya corriendo — cortar la red y relanzar la app no sirve para este build debug específicamente, porque sin red tampoco puede cargar su propio bundle JS desde Metro; hace falta cortar la red con la app *ya* corriendo). Con la lista ya cargada, se forzó una fetch de paginación real (scroll hasta el final) con la red cortada: `OfflineBanner` se mostró, la fetch fallida no reemplazó la lista, y apareció "Couldn't refresh, showing saved data" en vez de `ErrorState`. Además de la verificación manual, se agregó cobertura automatizada real en `src/store/offlineCache.integration.test.tsx` — a diferencia de todos los demás tests (que usan `testUtils.tsx`'s `renderWithProviders`, sin persistencia, a propósito), este usa el store real (`createAppStore()`, factory nueva en `store/index.ts` extraída específicamente para esto) con `redux-persist`/`AsyncStorage` reales: una sesión escribe datos de verdad al mock de `AsyncStorage`, una segunda sesión (store nuevo, misma `AsyncStorage`) los lee con la red simulando falla total, confirmando que el título aparece instantáneo sin nunca mostrar `LoadingState`.
- ~~Confirmar `TMDB_ACCESS_TOKEN` y que el Read Access Token funciona con `Authorization: Bearer`~~ — **resuelto**: confirmado con una key real, toda la app funcionando end-to-end contra la API real (incluyendo los 4 flujos de Maestro).
- ~~Verificar shared element transitions sobre `native-stack`~~ — **resuelto**: funciona correctamente, no hizo falta el fallback de cross-fade documentado en "Animaciones".
- ~~Confirmar que `cssInterop` registra bien los componentes `Animated.*`~~ — **resuelto**: usado en `Poster`, `FavoriteButton`, `OfflineBanner`, `RefreshIndicator` sin problemas.
- ~~Verificar que `install-expo-modules` funciona sobre un proyecto RN CLI recién inicializado~~ — **resuelto**: usado repetidamente sin fricción (`expo-image` en el paso 5, `expo-font`/`@expo/vector-icons` después).
- ~~Confirmar el estado de Fabric/New Architecture de `react-native-localize`~~ — **resuelto implícitamente**: usado durante todo el desarrollo (detección de idioma) sin ningún síntoma de incompatibilidad.
