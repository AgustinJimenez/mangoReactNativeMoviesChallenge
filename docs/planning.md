...
- **`/movie/popular` y `/tv/popular` no soportan `with_genres` ni `sort_by`** — según la propia doc de TMDB, esos endpoints son "básicamente un discover call detrás de escena" pero con parámetros fijos (solo `page`/`language`/`region`). Para que el filtro de género y el orden funcionen de verdad (no solo decorativos), `getPopularMovies`/`getPopularTv` (`api/tmdbApi.ts`) pasaron de `/movie/popular` y `/tv/popular` a `/discover/movie` y `/discover/tv`, que sí soportan ambos parámetros. Con `sort_by: 'popularity.desc'` por default (el mismo default de `/movie/popular`), el cambio es un no-op hasta que el usuario elige un filtro u orden.
- **`SortBy`** (`'popularity' | 'rating' | 'newest' | 'title'`) es un tipo compartido movie/tv, pero el parámetro TMDB real difiere por tipo: movies ordenan por `primary_release_date`, tv por `first_air_date` (fecha de estreno vs. fecha de primer episodio). `toMovieSortByParam`/`toTvSortByParam` en `tmdbApi.ts` hacen ese mapeo.
...
- **`append_to_response=credits,videos,recommendations`** en `getMovieDetails`/`getTvDetails` (`api/tmdbApi.ts`) — TMDB permite pedir sub-recursos relacionados en la misma llamada de detalles en vez de tres requests separados (`/movie/{id}/credits`, `/videos`, `/recommendations`). Mismo costo de red que antes, mucha más data.
...
- **Cast**: `CastMember[]` capado a `MAX_CAST_MEMBERS = 10` (constante en `tmdbApi.ts`) — es una tira de "elenco principal" en la página de detalles, no una lista de créditos completa; no hay pantalla de "ver todos" (se decidió no construir esa UI sin la navegación real detrás).
...
- ~~Decidir si "top_rated" o "popular" es el endpoint por defecto~~ — **resuelto**: `popular` (ver `api/tmdbApi.ts`).
- ~~Definir criterio exacto de normalización `Media`~~ — **resuelto**: implementado y testeado (`api/tmdbApi.ts` + `tmdbApi.test.ts`, paso 15).
