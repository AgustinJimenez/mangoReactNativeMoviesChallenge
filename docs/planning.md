...
- ~~Verificar que NativeWind compila sin fricción en Android e iOS~~ — **parcial**: confirmado extensivamente en Android; iOS nunca se compiló en esta máquina (sin Xcode instalado, ver README "Known limitations").
- **TODO (verificación manual pendiente)**: probar a mano el escenario completo de caché offline (cargar listado, cerrar app, modo avión, reabrir, ver listado cacheado + aviso de "no se pudo actualizar" en vez de `ErrorState`). Se verificó `OfflineBanner` alternando wifi real (paso 14), pero no específicamente el caso de datos servidos desde caché sin red. Lo va a probar el usuario directamente en el emulador.
